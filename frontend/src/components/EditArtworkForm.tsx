...
import { uploadImage, updateArtwork, fetchExhibitors } from '../api';
...
function splitMedia(artwork: Artwork) {
  const items: ArtworkMedia[] = artwork.media && artwork.media.length > 0
    ? artwork.media
    : artwork.imageUrl ? [{ type: 'image' as const, url: artwork.imageUrl }] : [];
  const primary = items.length > 0 ? items[0].url : artwork.imageUrl || '';
  const rest = items.slice(1).map(m => ({ type: m.type, url: m.url }));
  return { primaryUrl: primary, additional: rest };
}

export default function EditArtworkForm({ artwork, token, onClose, onUpdated }: EditArtworkFormProps) {
  const attrs = artwork.attributes || {};
  const initial = splitMedia(artwork);
  ...
  const [additionalMedia, setAdditionalMedia] = useState<MediaItem[]>(initial.additional);
  ...
  const [selectedContributors, setSelectedContributors] = useState<string[]>(
    () => artwork.contributors?.map(c => c.exhibitorId) || []
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateArtwork(token, artwork.id, body);
      onUpdated();
      onClose();
    } catch (err: any) {
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-zinc-900 rounded-none sm:rounded-3xl w-full h-full sm:h-auto max-w-5xl overflow-hidden shadow-2xl flex flex-col max-h-[100vh] sm:max-h-[90vh]">
        <div className="px-6 sm:px-8 py-5 flex items-center justify-between sticky top-0 z-10 bg-white dark:bg-zinc-900 border-b sm:border-b-0 border-zinc-100 dark:border-zinc-800">
          <div>
            ...
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5 truncate max-w-[200px]">{artwork.title}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-8 pb-4 sm:pb-8 pt-4 sm:pt-0">
          {error && (
          ...
          <form id="edit-artwork-form" onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-5 gap-6 sm:gap-10">
                ...
                <div className="group relative aspect-square bg-zinc-50 dark:bg-zinc-950 border-2 border-dashed border-zinc-200 dark:border-zinc-700 rounded-3xl overflow-hidden hover:border-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all cursor-pointer flex flex-col items-center justify-center text-center p-6">
                  ...
                  {primaryPreview ? (
                    ...
                    <div className="space-y-2 pointer-events-none">
                      ...
                      <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Change Image</p>
                    ...
                    {selectedContributors.map(id => {
                      ...
                      return ex ? (
                        <span key={id} className="inline-flex items-center gap-1 text-[10px] font-semibold bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 px-2 py-1 rounded-full">
                          {ex.name}
                          <button onClick={() => toggleContributor(id)} className="text-zinc-400 dark:text-zinc-500 hover:text-zinc-700">✕</button>
                        </span>
                      ) : null;
                    })}
                  </div>
